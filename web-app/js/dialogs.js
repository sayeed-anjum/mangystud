$.Class.extend("Dialog", {}, {
	init : function(name, manager) {
		...
		this.el.live('keypress', function(e) {
		    if (e.which == 13) {
		    	$(this).parent().find('button:first').trigger('click');
		        e.preventDefault();
		    }
		});
		return this;
	...
	find : function(s, items, key) {
		var found = false;
		s = s.toLowerCase();
		$.each(items, function(index, item) {
			if (item[key].toLowerCase() == s) {
				found = true;
				return false;
			}
		});
	...
	getCurrentRealmId : function() {
		var tiddler = this.manager.currentTiddler;
		return $('.realm_select select', tiddler).val();
	}
});
...
Dialog.extend("RealmDialog", {}, {
	...
	isValid : function() {
		var name = $('[name="name"]', this.el).val().trim();
		if (name === "") {
			alert('Please enter a name');
			return false;
		}
		var realms = this.manager.getRealms();
		if (this.find(name, realms, 'name')) {
			alert('A realm \'' + name + '\' already exists');
			return false;
		}
		return true;
	},
...
Dialog.extend("ContextDialog", {}, {
	...
	isValid : function() {
		...
		var contexts = this.manager.getContexts(this.getCurrentRealmId());
		if (this.find(name, contexts, 'name')) {
			alert('A context \'' + name + '\' already exists');
			return false;
		}
		return true;
	},
...
Dialog.extend("ActionDialog", {}, {
	...
	updateContext : function(obj) {
		...
		var state = "";
		if ($(p).hasClass('dc_state_Next'))
			state = 'Next';
		if ($(p).hasClass('dc_state_Future'))
			state = 'Future';
		if ($(p).hasClass('dc_state_WaitingFor'))
			state = 'WaitingFor';
	...
	onSuccess : function(data, textStatus) {
		...
		manager.raiseEvent('actionUpdate', {
			event : 'newAction',
			data : data,
			id : data.action.id
		});
...
Dialog.extend("AreaDialog", {}, {
	...
	isValid : function() {
		...
		var areas = this.manager.getAreas(this.getCurrentRealmId());
		if (this.find(name, areas, 'name')) {
			alert('An area \'' + name + '\' already exists');
			return false;
		}
		return true;
	},
...
Dialog.extend("ContactDialog", {}, {
	...
	isValid : function() {
		...
		var contacts = this.manager.getContacts(this.getCurrentRealmId());
		if (this.find(name, contacts, 'name')) {
			alert('An contact \'' + name + '\' already exists');
			return false;
		}
		return true;
	},
...
Dialog.extend("TicklerDialog", {}, {
	...
	onSuccess : function(data, textStatus) {
		this.close();
		manager.raiseEvent('ticklerUpdate', {
			event : 'newTickler',
			data : data,
			id : data.tickler.id
		});
...
Dialog.extend("ProjectDialog", {}, {
	...
	updateContext : function(obj) {
		var p = $(obj).parent();
		if ($(p).hasClass('dc_status_Active'))
			$('[name=status]', this.el).val('Active')
		if ($(p).hasClass('dc_status_Someday'))
			$('[name=status]', this.el).val('Someday')
	},
	...
	onSuccess : function(data, textStatus) {
		this.close();
		manager.raiseEvent('projectUpdate', {
			event : 'newProject',
			data : data,
			id : data.project.id
		});
...
Dialog.extend("ReferenceDialog", {}, {
	...
	onSuccess : function(data, textStatus) {
		this.close();
		manager.raiseEvent('referenceUpdate', {
			event : 'newReference',
			data : data,
